...
# using decorator
"""
from singleton_skill_builder import SingletonSkillBuilder
sb = SingletonSkillBuilder()
@sb.request_handler(can_handle_func=is_intent_name("InventoryIntent"))
def inventory_intent_handler(handler_input):
    logger.info("In InventoryIntentHandler")
    # service slot resolution
    try:
        svc = handler_input.request_envelope.request.intent.slots['service']
